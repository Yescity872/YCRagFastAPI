...
class NearbySpotBot:
    def __init__(self, city: str):
        ...
        self.namespace = f"NearbySpot-{self.city.title()}" if self.city == "rishikesh" else f"nearbyspot-{self.city}"
        try:
    ...
    def _format(self, query: str, docs: List[Document]) -> str:
        context = "\n\n".join([
            f"Spot: {d.metadata.get('places','N/A')}\nCategory: {d.metadata.get('category','N/A')}\nDescription: {d.metadata.get('description','N/A')}" for d in docs
        ])
        prompt = f"""
You are a nearby tourist spot guide for {self.city.title()}. Use only context. Max 5 results.
...
Format:
1. Spot Name (Category) - Short Highlight
2. ...
"""
    ...
    def nearbyspot_bot(self, query: str, k: int = 2) -> Dict[str, Any]:
