...
class ItineraryBot:
    def __init__(self, city: str):
        ...
        self.namespace = f"Itinerary-{self.city.title()}" if self.city == "rishikesh" else f"itinerary-{self.city}"
        try:
    ...
    def _format(self, query: str, docs: List[Document]) -> str:
        context = "\n\n".join([
            f"Day1: {d.metadata.get('day1','')}\nDay2: {d.metadata.get('day2','')}\nDay3: {d.metadata.get('day3','')}" for d in docs if any([d.metadata.get('day1'), d.metadata.get('day2'), d.metadata.get('day3')])
        ])
        prompt = f"""
You are an itinerary planner for {self.city.title()}. Use only context. If query asks for 1-day, 2-day, or 3-day plan, compile concise ordered bullet list. Avoid hallucination.
...
"""
    ...
    def itinerary_bot(self, query: str, k: int = 2) -> Dict[str, Any]:
