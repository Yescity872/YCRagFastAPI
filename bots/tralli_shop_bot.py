...
class ShopBot:
    def __init__(self, city: str):
        ...
        self.namespace = f"Shop-{self.city.title()}" if self.city == "rishikesh" else f"shop-{self.city}"
        try:
    ...
    def _query(self, query: str, k: int = 2):
        if not self.index:
            return []
        try:
    ...
    def _format(self, query: str, docs: List[Document]) -> str:
        context = "\n\n".join([
            f"Shop: {d.metadata.get('shops','N/A')}\nFamous For: {d.metadata.get('famousFor','N/A')}\nPrice Range: {d.metadata.get('priceRange','N/A')}" for d in docs
        ])
        prompt = f"""
You are a shopping assistant for {self.city.title()}. Use only context. Max 5 results.
...
Format:
1. Shop Name - Famous For - Price Range
2. ...
"""
    ...
    def shop_bot(self, query: str, k: int = 2) -> Dict[str, Any]:
