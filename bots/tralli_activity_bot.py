...
class ActivityBot:
    ...
    def _format(self, query: str, docs: List[Document]) -> str:
        context = "\n\n".join([
            f"Activity: {d.metadata.get('topActivities','N/A')}\nBest Places: {d.metadata.get('bestPlaces','N/A')}\nFee: {d.metadata.get('fee','N/A')}\nDescription: {d.metadata.get('description','N/A')}" for d in docs
        ])
        prompt = f"""
You are an adventure & activities expert for {self.city.title()}. Use only context. Max 5 results.
...
Format:
1. Activity Name - Key Spot(s) - Fee / Highlight
2. ...
"""
    ...
    def activity_bot(self, query: str, k: int = 2) -> Dict[str, Any]:
